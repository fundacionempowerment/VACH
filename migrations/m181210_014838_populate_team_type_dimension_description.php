...
/**
 * Class m181210_014838_populate_team_type_dimension_description
 */
class m181210_014838_populate_team_type_dimension_description extends Migration {
    ...
    const descriptions = [
        ...
        2 => [
            ...
            4 => 'Resolver eficazmente diferencias en distintas posiciones, modificando posturas personales y/ o grupales en beneficio de todas las partes involucradas.',
            5 => 'Capacidad para promover e implementar los cambios requeridos a fin de mantener la ventaja competitiva de la empresa en el mercado.',
    ...
    public function safeUp() {
        foreach (self::descriptions as $level => $data) {
            foreach ($data as $dimension => $text) {
                $this->populate($level, $dimension, $text);
            }
        }
    }

    private function populate($level, $dimension, $text) {
        $this->update('team_type_dimension', [
            'description' => $text,
        ], [
            'order' => $dimension,
            'level' => $level
